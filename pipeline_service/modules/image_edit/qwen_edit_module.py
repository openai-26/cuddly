...
class QwenEditModule(QwenManager):
    ...
    def _combine_images_for_editing(self, 
                                    bg_removed_image: Image.Image, 
                                    original_image: Image.Image) -> Image.Image:
        """
        Combine background-removed and original images for dual-image editing.
        The model will use the background-removed structure and restore details from original.
        
        Args:
            bg_removed_image: Image with background removed (structure guide)
            original_image: Original image (detail reference)
            
        Returns:
            Combined image for editing
        """
        # Resize both images to the same size for processing
        bg_removed = self._prepare_input_image(bg_removed_image)
        original = self._prepare_input_image(original_image)
        
        # Ensure both images are same size
        if bg_removed.size != original.size:
            original = original.resize(bg_removed.size, Image.Resampling.LANCZOS)
        
        # Create a side-by-side composition or use the bg_removed as primary
        # For now, we'll use the bg_removed image as primary input
        # and the model will be instructed via prompt to restore details
        return bg_removed
    ...
    def _run_edit_pipe(self,
                       ...
                       **kwargs):
        # If we have a reference image, combine them appropriately
        if reference_image is not None:
            # Prepare the composite for dual-image editing
            prompt_image = self._combine_images_for_editing(prompt_image, reference_image)
        else:
    ...
    def edit_image(self, prompt_image: Image.Image, seed: int, reference_image: Optional[Image.Image] = None):
        """ 
        Edit the image using Qwen Edit.

        Args:
            prompt_image: The prompt image to edit (typically background-removed image).
            seed: Random seed for generation.
            reference_image: Optional reference image (typically original image) to restore details from.
        ...
        try:
            ...
            # Run the edit pipe with optional reference image
